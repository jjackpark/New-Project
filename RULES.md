...
## 7. 백로그 대시보드 (`scripts/dashboard/`)

- `node scripts/dashboard/server.js` (기본 포트 4600, `PORT` 환경변수로 변경 가능)로 실행하는 조회 전용 웹 대시보드. npm 의존성이 없고 Node 내장 `http`/`fs`만 쓴다 — 기술 스택(T001) 결정과 무관하게 지금 바로 쓸 수 있다.
- 서버는 요청마다 `backlog.json`과 `tasks/*.md`를 다시 읽어 KPI, 단계별 진행률, 상태별 개수, 각 태스크의 의존성/역의존성, 연결 문서 본문을 계산해 `/api/backlog`로 내려준다. 프런트(`public/js/main.js`)는 이 엔드포인트를 4초마다 폴링해 화면을 갱신한다(진짜 서버 푸시가 아니라 폴링 기반 "실시간"임을 유의).
- **쓰기 기능이 없다.** 이 대시보드로는 상태/필드를 바꿀 수 없다 — 그런 목적의 API를 추가하지 않는다. 모든 변경은 §2의 `backlog-cli.js`를 통해서만 한다(순환 의존성 검사 등 검증 로직이 CLI에만 있기 때문).
- 디자인은 SPEC.md/CLAUDE.md의 다크 네이비 사이드바 + 화이트 카드 + 색상 뱃지 톤을 따른다. 새 화면 요소를 추가할 때도 이 톤을 유지한다.
- 프런트 JS는 `public/js/` 아래 `api.js`(fetch) / `utils.js`(포맷팅·뱃지·간이 마크다운) / `render-overview.js`(KPI·진행률·상태칩) / `render-tasks.js`(사이드바·표·상세패널) / `main.js`(상태 관리·폴링)로 나뉘어 있다. 300줄 제한(§5)에 걸리지 않도록 새 기능은 이 분리를 유지하며 추가한다.

## 8. 제품 규칙 (SPEC.md 요약 — 구현 시 임의로 벗어나지 않는다)
